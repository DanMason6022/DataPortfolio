...
# Bin setup
num_bins = 40
bin_edges = np.linspace(df['avg_msrp'].min(), df['avg_msrp'].max(), num_bins + 1)
bin_width = bin_edges[1] - bin_edges[0]

# Assign bins and midpoints
df['bin'] = pd.cut(df['avg_msrp'], bins=bin_edges, include_lowest=True)
...
bar_df = df.groupby('bin_mid', dropna=True).agg({
    ...
    'avg_msrp': 'count'
}).reset_index().rename(columns={'avg_msrp': 'count'})

# KDE for bell curve
msrp = df['avg_msrp'].values
kde = gaussian_kde(msrp, bw_method=0.3)
x_vals = np.linspace(msrp.min(), msrp.max(), 500)
kde_vals = kde(x_vals) * len(msrp) * bin_width  # scale for histogram comparison

# Bar trace using secondary color (#ff69b4)
bar_trace = go.Bar(
    x=bar_df['bin_mid'],
    y=bar_df['count'],
    text=bar_df['label'],
...
percentiles = np.percentile(msrp, np.arange(10, 100, 10))
percentile_lines = []
percentile_labels = []

base_y = max(kde_vals) * 1.5
...
fig.show()
fig.write_html("msrp_distribution.html")
